...
function milestones(selector) {
  ...
  function render(data) {
    ...
    var timelineEnter = timeline.enter().append('div')
    ...
    var rightMargin = 11;
    var selectorWidth = parseFloat(select(selector).style('width')) - rightMargin;
    ...
    var allKeys = nestedData.reduce(function (keys, timeline) {
      ...
      t.map(function (d) { return keys.push(d.key); });
      return keys;
    }, []);

    var domain = (typeof range !== 'undefined')
      ? range.map(aggregateFormatParse)
      : extent(allKeys, function (d) { return aggregateFormatParse(d); });
    ...
    if (useLabels) {
      var label = groupMerge.selectAll('.' + cssLabelClass).data(function (d) { return [d]; });

      var labelMerge = label.enter().append('div')
        ...
        .classed(cssAboveClass, function (d) { return isAbove(d.index); });

      var text = labelMerge.selectAll('.' + cssTextClass).data(function (d) { return [d]; });
      ...
      var textEnter = text.enter().append('div')
        ...
        .style('width', function (d) {
          ...
          if (typeof nextItem !== 'undefined') {
            ...
            if ((itemNumTotal - itemNum) === 1) {
              ...
              if (typeof previousItem !== 'undefined') {
                var offsetPreviousItem = x(aggregateFormatParse(previousItem.key));
          ...
          var labelRightMargin = 6;
          var finalWidth = Math.min(labelMaxWidth, (Math.max(0, availableWidth - labelRightMargin)));
          ...
          nestedNodes.forEach(function (d) {
            ...
            nodes.forEach(function (node) {
              ...
              if (
                ...
                var padding = isAbove(index) ? 'padding-bottom' : 'padding-top';
    ...
    timelineMerge.each(function (d, i, node) {
      var margin = 10;
      var maxAboveHeight = max(select(node[i]).selectAll('* .' + cssAboveClass)._groups[0], function (d) { return d.offsetHeight; });
      var maxBelowHeight = max(select(node[i]).selectAll('* :not(.' + cssAboveClass + ')')._groups[0], function (d) { return d.offsetHeight; });
