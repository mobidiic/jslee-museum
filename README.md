...
<br />
위에서 정한 사이트맵 대로, 메인페이지를 먼저 구축하였다.
주요 이슈는 슬라이딩 배너 기능을 구현하는 것과 기본 토글네비게이션 바를 만드는 것, 그리고 sticky menu bar 였다.
<br /><br />
슬라이딩 배너를 만들기 위해 레퍼런스를 찾아보았지만, 대부분이 제이쿼리를 이용한 플러그인 형태로 구성되어 있었다. 본 페이지에서는 도훈님이 만든 jquery.touchslider 플러그인을 사용하였다. 이후에 이 의존성을 없애기 위해 순수 자바스크립트 코드로 고칠 계획이 있다.
<br /><br />
이러한 방식으로 구현한 메인 페이지는 다음과 같다.
<br /><br />

![demo_main1](./imgs/reports/demo_mainPage1.png)
![demp_main2](./imgs/reports/demo_mainPage2.png)

이후의 각 네비게이션 메뉴에 따른 상세페이지는 리액트 라우터를 활용하여 multiple pages on SPA 를 구현할 예정이다.

<br /><br />


리액트 라우터 v4를 활용하여 각 페이지들을 연결한 다음 먼저 처음으로 에러 페이지를 만들었다.
